...
Citizen.CreateThread(function()
    while true do
        ...
        if dstH <= 2.5 and IsControlJustReleased(0, 38) then
            ESX.TriggerServerCallback('araf-blacksmith:getItemCount', function(cb)
                if cb >= araf.requiredItemCount then
                    ...
                    if araf.useSkillBar then
                        ...
                        if finished ~= 100 then
                            ...
                            TriggerServerEvent('araf-blacksmith:deleteItem', araf.iron, 2)
                            ClearPedTasks(ply)
                        else
                            TriggerServerEvent('araf-blacksmith:giveHeatedItem')
                            ClearPedTasks(ply)
                        end
                ...
                if cb >= araf.requiredItemCount then
                    ...
                    if araf.useSkillBar then
                        ...
                        if finished ~= 100 then
                            ...
                            TriggerServerEvent('araf-blacksmith:deleteItem', araf.heatedIron, 2)
                            ClearPedTasks(ply)
                        else
                            TriggerServerEvent('araf-blacksmith:giveWroughtedItem')
                            ClearPedTasks(ply)
                        end
                    elseif araf.useProgbar then
        ...
        for k, v in pairs(araf.mining) do
            ...
            if dstM <= 2.5 and IsControlJustReleased(0, 38) then
                ...
                playAnim('melee@large_wpn@streamed_core', 'ground_attack_on_spot', -1, 49)
                if araf.useSkillBar then
                    ...
                    if finished ~= 100 then
                        ...
                        TriggerServerEvent('araf-blacksmith:giveMiningItem')
                        ClearPedTasks(ply)
                        DetachEntity(pickaxe, 1, true)
                        DeleteEntity(pickaxe)
                        DeleteObject(pickaxe)
