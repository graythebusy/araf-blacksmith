...
AddEventHandler('araf-blacksmith:giveHeatedItem', function()
    ...
    local ironCount = xPlayer.getInventoryItem(araf.iron).count
...
AddEventHandler('araf-blacksmith:giveWroughtedItem', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local ironCount = xPlayer.getInventoryItem(araf.heatedIron).count
    ...
    if ironCount >= araf.requiredItemCount then
        ...
        elseif 50 < chance and chance <= 100 then
